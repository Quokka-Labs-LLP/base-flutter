  ...
  InternetCheckerBloc() : super(InternetCheckerState()) {
    on<ListenInternetConnectionEvent>(
      (final event, final emit) =>
          emit(state.copyWith(event.isConnected, event.internetType)),
    );

    on<CheckInternetConnectionEvent>((final event, final emit) async {
...
class InternetCheckerState {
  ...
  InternetCheckerState({
    ...
    this.internetType = InternetType.none,
  });
